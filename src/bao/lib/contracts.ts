...
export interface FarmableSupportedPool extends SupportedPool {
  ...
  export class Contracts {
	...
	constructor(
	  ...
	  this.roboVault = new this.web3.eth.Contract(VaultAbi as AbiItem[])
	...
	setProvider(provider: provider, networkId: number): void {
	  ...
	  if (networkId == 1) {
		...
		setProvider(this.roboVault, contractAddresses.roboVault[networkId])
	...
	setDefaultAccount(account: string): void {
	  ...
	  this.roboVault.options.from = account
